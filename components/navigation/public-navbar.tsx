        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-gray-700 hover:text-[#3E5F44] transition-colors duration-200 font-medium"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            <Link href="/auth/login">
              <Button variant="ghost" className="text-gray-700 hover:text-[#3E5F44]">
                Sign In
              </Button>
            </Link>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="block px-3 py-2 text-gray-700 hover:text-[#3E5F44] font-medium"
              ...
              <div className="pt-4 space-y-2">
                ...
                <Link href="/auth/login" className="block">
                  <Button variant="ghost" className="w-full text-gray-700 hover:text-[#3E5F44]">
                    Sign In
                  </Button>
                </Link>
                <Link href="/auth/sign-up" className="block">
                  <Button className="w-full bg-[#3E5F44] hover:bg-[#2f4a35] text-white">Get Started</Button>
                </Link>
              </div>
            </div>
          </div>
